...
namespace Razor.Service{
    // Tất cả các lỗi của identity ở đây
    public class AppIdentityErrorDescriber : IdentityErrorDescriber
    {
        ...
        public override IdentityError DuplicateEmail(string email) // Thông Báo email bị trùng
        {

            return base.DuplicateEmail(email);
        }

        public override IdentityError DuplicateRoleName(string role) // Thông Báo role bị trùng
        {
            var er = base.DuplicateRoleName(role);
            return new IdentityError(){
                Code = er.Code,
                Description = $"Role có tên {role} đã bị trùng"
